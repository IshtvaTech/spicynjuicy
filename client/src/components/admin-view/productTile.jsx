      ...
      <Card className="w-full max-w-sm mx-auto">
        <div>
          ...
          <CardFooter className="flex justify-between items-center">
            <Button className=" bg-yellow-800 hover:bg-yellow-700 cursor-pointer" onClick={()=>{
              setOpenCreateProductsDialog(true)
              setCurrentEditedId(product?._id)
              setFormData(product)
            }}>Edit</Button>
            <Button className="bg-yellow-800 hover:bg-yellow-700 cursor-pointer" onClick={()=>handleDelete(product?._id)}>Delete</Button>
          </CardFooter>
        </div>
      </Card>
    </div>
